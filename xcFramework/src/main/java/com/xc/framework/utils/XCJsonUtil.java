		...
		try {
			if (jsonObject != null && objectClass != null) {
				...
				if (info != null) {
					...
					List<String> fieldNameList = XCBeanUtil.getFieldNameList(objectClass);
					if (fieldNameList != null && !fieldNameList.isEmpty()) {
						for (int i = 0; i < fieldNameList.size(); i++) {
							String name = fieldNameList.get(i) != null ? fieldNameList.get(i) : "";
							if (!XCStringUtil.isEmpty(name) && !jsonObject.isNull(name)) {
								String value = jsonObject.optString(name, "");
								XCBeanUtil.invokeSetMethod(info, name, !XCStringUtil.isEmpty(value) ? value : "");
							}
						}
					}
				}
